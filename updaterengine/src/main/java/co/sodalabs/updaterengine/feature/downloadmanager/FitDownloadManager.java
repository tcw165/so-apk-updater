 ...
 */
public class FitDownloadManager implements DownloadManager {
     ...
     */
    public FitDownloadManager(boolean loggingEnabled) {
     ...
     */
    public FitDownloadManager(Handler callbackHandler) throws InvalidParameterException {
     ...
     **/
    public FitDownloadManager(int threadPoolSize) {
    ...
    public int add(DownloadRequest request) throws IllegalArgumentException {
        checkReleased("add(...) called on a released FitDownloadManager.");
        if (request == null) {
    ...
    public int cancel(int downloadId) {
        checkReleased("cancel(...) called on a released FitDownloadManager.");
        return mRequestQueue.cancel(downloadId);
    }

    @Override
    public void cancelAll() {
        checkReleased("cancelAll() called on a released FitDownloadManager.");
        mRequestQueue.cancelAll();
    }

    @Override
    public int pause(int downloadId) {
        checkReleased("pause(...) called on a released FitDownloadManager.");
        return mRequestQueue.pause(downloadId);
    }

    @Override
    public void pauseAll() {
        checkReleased("pauseAll() called on a released FitDownloadManager.");
        mRequestQueue.pauseAll();
    }

    @Override
    public int query(int downloadId) {
        checkReleased("query(...) called on a released FitDownloadManager.");
